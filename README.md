...
Sawaganiは、記憶を持ち、生命が宿るように感じられるエージェントを目指すプロジェクトです。(製作中)
...
```bash
sawagani start   # OS 監視付き統合常駐を登録して開始
sawagani status  # 実行状態とログの場所を表示
sawagani stop    # OS 監視付き常駐の登録を解除
```

`sawagani start` は macOS では LaunchAgent、Linux では systemd user service を登録します。
ハートビートと Discord Bot（`[discord] enabled = true` の場合）を1つのプロセスで同居させ、
ログイン時に自動開始し、クラッシュ時は OS が再起動します。動作確認やデバッグでは、前面実行もできます。

```bash
sawagani tick                                  # 1回だけ実行（テスト用）
sawagani serve --interval 1800                 # 統合常駐サービスを前面で実行
...
- **`schedule.md`** … 未来の作業予約を書く自己予約テーブル。

Sawagani が管理するファイル／ディレクトリは次のとおりです。

- **`MEMORY.md`** … Sawagani の**記憶**。起動と停止を跨いで残り、毎ティックの判断の土台になる。「いつ・何をしたか」の追記は LLM ではなく Sawagani 本体が行う。
- **`web-data/`** … Web から取得したデータの保存先。`config.toml` で変更できる。
- **`STOP`** … 一時停止用のキルスイッチ。作成するとプロセスは生きたままティックと予約発火を止める（`touch STOP`）。削除すると再開する。
- **`sawagani.log`** … バックグラウンド実行の標準出力・標準エラー。

内部実装として `src/sawagani/settings.py` がありますが、これは `config.toml` を読むためのコードです。通常は編集しません。

記憶と状態のファイル（`tasks.md` / `schedule.md` / `MEMORY.md` / `STOP` / `config.toml`）は実行ディレクトリ基準です。
別の場所で動かす場合は `SAWAGANI_HOME` でデータディレクトリを指定できます。

## 予約（`schedule.md`）

`schedule.md` に予約を書くと、次回以降のティック開始時に `tasks.md` へ追加されます。実行精度はティック間隔に依存します。
...
- `at:<ISO8601>` はワンショット予約です。
- `cron:<式>` は繰り返し予約です。v1では一般的な5フィールドcron（`*`、数値、カンマ、月/曜日名）を扱います。
- `STOP` ファイルで一時停止している間は予約も消化されません。
...
# channel_id = 123456789012345678  # 指定すると非 IDLE の作業結果を通知
...
- 暴走防止: 1ティックのツール上限・総回数上限・間隔の下限・`STOP` による一時停止。
- `config.toml` はローカル運用ファイルとして `.gitignore` しています。Bot Token などの秘密情報は引き続き書かず、
  環境変数または `~/.config/sawagani/secrets`（mode 0600）で渡してください。
...
```bash
mkdir -p ~/.config/sawagani
printf 'SAWAGANI_DISCORD_BOT_TOKEN=%s\n' '手順1で取得したトークン' > ~/.config/sawagani/secrets
chmod 600 ~/.config/sawagani/secrets
```

一時的に前面実行したい場合だけ、環境変数でも渡せます。

```bash
export SAWAGANI_DISCORD_BOT_TOKEN="..."
sawagani discord start   # Discord のみを前面で起動（デバッグ用）
```

通常運用は `sawagani start` で統合常駐として起動します。
...
`/sawagani task` は直接実行せず、まず `tasks.md` に追記します。統合常駐中は待機中のハートビートを即座に起こし、
通常の権限境界の中で処理されます。`channel_id` を指定している場合、非 IDLE の作業結果はそのチャンネルへ通知されます。
...
会話モードでは Sawagani が `tasks.md` / `schedule.md` / `MEMORY.md` / `config.toml` を読んで状況を説明し、
`tasks.md` へのタスク追加、`schedule.md` への予約追加、`config.toml` の編集も行えます。`config.toml` の変更は、
実行中のデーモンや現在の会話セッションには即時反映されず、次回 `sawagani start` 起動時に反映されます。

強い機能なので、必要に応じて `allowed_user_ids` で会話できるユーザーを制限してください。会話の文脈を消したいときは、メンションまたは DM で `リセット` と送ると、そのチャンネルの会話セッションを作り直します。

### うまくいかないとき

- **`sawagani start` したのに動かない** … macOS の LaunchAgent は最小環境で動くため、登録時の `PATH` を plist に埋め込んでいます。`claude` CLI の場所が変わった場合は `sawagani start` を再実行してください。
- **macOS でログアウト中に動かない** … LaunchAgent は GUI ログイン中のユーザーセッションで動きます。ログアウト中も動かしたい用途は Linux systemd user service と linger を使ってください。
- **Linux でログアウト後に動かない** … systemd user service をログアウト後も動かすには `loginctl enable-linger $USER` が必要な場合があります。
...
- **`SAWAGANI_DISCORD_BOT_TOKEN is not set`** … 環境変数または `~/.config/sawagani/secrets` が未設定です。手順5を確認してください。
- **`chmod 600` を求められる** … secrets ファイルが他ユーザーから読める権限です。`chmod 600 ~/.config/sawagani/secrets` を実行してください。
