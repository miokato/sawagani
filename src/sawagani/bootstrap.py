...
[discord]
# Discord から Sawagani を操作する場合に true。
# Bot Token は config.toml には書かず、環境変数または ~/.config/sawagani/secrets に置く。
enabled = false
# メンション/DM で Sawagani と会話する場合に true。
# 有効にすると会話から config.toml / tasks.md / schedule.md を編集できるため、allowed_user_ids で利用者を絞る。
conversation = false
# channel_id を指定すると、自律作業の非 IDLE 結果をそのチャンネルへ通知する。
