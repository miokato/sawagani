...
class DiscordSettings:
    """Discord Bot 連携の設定値。Token は load_bot_token() で別管理する。"""
...
編集できる場所は `{config_path}` / `{tasks_path}` / `{SCHEDULE_FILE}` / `{web_data_dir_path}` 以下だけです。
...
def secrets_path() -> Path:
    """Discord Bot Token を置く secrets ファイルの絶対パスを返す。"""
    config_home = os.environ.get("XDG_CONFIG_HOME")
    if config_home:
        return Path(config_home) / "sawagani" / SECRETS_FILE
    return Path.home() / ".config" / "sawagani" / SECRETS_FILE
...
def parse_key_value_secret(text: str, key: str) -> str | None:
    """KEY=VALUE 形式の secrets テキストから指定 key の値を取り出す。"""
...
def load_bot_token() -> str:
    """Discord Bot Token を環境変数または 0600 の secrets ファイルから読む。"""
    ...
    if path.is_file():
        ...
        file_token = parse_key_value_secret(path.read_text(encoding="utf-8"), DISCORD_BOT_TOKEN_ENV)
        if file_token:
            return file_token

    raise RuntimeError(
        f"{DISCORD_BOT_TOKEN_ENV} is not set. Set the environment variable or "
        f"write `{DISCORD_BOT_TOKEN_ENV}=...` to {path} with chmod 600."
    )
